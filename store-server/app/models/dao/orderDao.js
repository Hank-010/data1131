...
module.exports = {
  ...
  GetOrderGroup: async (user_id) => {
    ...
    let sql = 'select DISTINCT order_time from orders where user_id = ? order by order_time desc';
  ...
  AddOrder: async (length, data) => {
    ...
    const sql = `
        INSERT INTO orders (
        id, user_id, product_id, product_num, product_price, order_time, total_price
        ) VALUES ${placeholders}
    `;
